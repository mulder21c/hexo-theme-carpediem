...
const createConfig = () => {
    ...
    return {
        paths: {
            ...
            outputCss: path_1.default.join(projectRoot, "source/css"),
            outputMap: path_1.default.join(projectRoot, "src/styles"),
            styles: path_1.default.join(projectRoot, "src/styles"),
            components: path_1.default.join(projectRoot, "src/components"),
        },
        filePatterns: {
        ...
        sass: {
            style: "compressed",
            loadPaths: [
                path_1.default.join(projectRoot, "src/styles"),
                path_1.default.join(projectRoot, "src/components"),
            ],
        },
        postcss: {
            preset: "default",
 ...
 */
function findScssFiles(searchPath) {
    return glob_1.glob.sync(`${config.paths.src}${searchPath}`);
}
/**
...
async function compileScss(filePath) {
    ...
    const isModule = path_1.default.basename(filePath).includes(".module.scss");
    const fileContent = fs_1.default.readFileSync(filePath, "utf-8");
    const contentWithAdditionalData = `${SCSS_IMPORTS_TEMPLATE}\n${fileContent}`;
    // Compile SCSS to CSS using sass compiler
    const result = sass_1.default.compileString(contentWithAdditionalData, {
 ...
 */
async function processWithPostcss(css, filePath, isModule) {
    const srcPath = path_1.default.resolve(config.paths.projectRoot, "src").replace(/\\/g, "/");
    ...
    const plugins = [(0, cssnano_1.default)({ preset: config.postcss.preset })];
    // For CSS modules, add the postcss-modules plugin to generate class name mappings
    if (isModule) {
        plugins.unshift((0, postcss_modules_1.default)({
 ...
 */
async function saveCssModulesMap() {
    (0, mkdirp_1.mkdirpSync)(config.paths.outputMap);
    fs_1.default.writeFileSync(path_1.default.join(config.paths.outputMap, "css-modules-map.json"), JSON.stringify(cssModulesMap, null, 2));
}
/**
 ...
 */
async function combineScssFiles(scssFiles) {
...
function writeCssFile(cssContent) {
    const outputPath = path_1.default.join(config.paths.outputCss, "style.css");
    fs_1.default.writeFileSync(outputPath, cssContent);
}
/**
...
async function renderScss() {
    ...
    try {
        ...
        (0, mkdirp_1.mkdirpSync)(config.paths.outputCss);
 ...
 */
async function optimizeCss(css, filePath) {
    try {
        const result = await (0, postcss_1.default)([(0, cssnano_1.default)({ preset: config.postcss.preset })]).process(css, {
            from: filePath,
        });
        return result.css;
    }
...
hexo.extend.renderer.register("css", "css", async function renderCSS(data) {
    const log = hexo?.log || console;
    try {
        const css = data.text || "";
